...
public class AuthorityController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        travelDirection = GameManager.instance.GetPlayerPos() - authorityTransform.position;
        travelDirection.y = 0;
    ...
    private void TestPlayerDetection()
    {
        ...
        RaycastHit info;
        if (Physics.Raycast(travelRay, out info, playerCaptureDistance) && info.collider == GameManager.instance.GetPlayer().GetComponent<Collider>())
        {
            GameManager.instance.OnPlayerCaptured();
        }
    }
    ...
    private void DirectAwayFromObstacles()
    {
        ...
        RaycastHit info;
        if (Physics.Raycast(travelRay, out info, 10.0F) && info.collider != this.GetComponent<Collider>() && info.collider != GameManager.instance.GetPlayer().GetComponent<Collider>())
        {
